...
#
# Provider packages (anthropic, bedrock, azure-identity), the Postgres
# driver, and the deep-memory embedder (chromadb+onnxruntime) are included
# so Docker users can use these providers/backends without requiring runtime
# lazy-install access to PyPI (often blocked in containerized envs).
#
# The editable link is created after the source copy below.
COPY pyproject.toml uv.lock ./
RUN touch ./README.md
RUN uv sync --frozen --no-install-project --extra all --extra messaging --extra anthropic --extra bedrock --extra azure-identity --extra postgres --extra deep-memory
