...
# DRY_RUN, WRITE_TFVARS, and CREATE_REPO must each be exactly 0 or 1. A typo
# (DRY_RUN=true, CREATE_REPO=yes) must not be silently coerced into an unsafe
# path, so reject anything else with a clear non-secret error naming the variable
# BEFORE any gcloud/docker side effect or live prerequisite command runs.
...
require_boolean_flag CREATE_REPO "$CREATE_REPO"
