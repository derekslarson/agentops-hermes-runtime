...
1. Durable Postgres + `pgvector` + full-text search backing the control-plane API (currently uses `LocalDeepMemoryBackend(partition=True)` in the api process — not durable across restarts, not shared across workers).
