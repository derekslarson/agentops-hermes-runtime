...
4. **HTTP adapter coverage is partial.** Compose currently wires HTTP adapters for curated memory, cron, artifacts, audit, and conversation routing. Sessions, skills, credentials/secrets, queue, run leases, worker registry, delivery, and deep memory need explicit remote/durable adapters or control-plane endpoints before the first version is complete.
