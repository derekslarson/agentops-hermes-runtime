...
- Extracted-signal boost parity: `RelationalMemoryRecordBackend` ingests extracted signals into both SQLite and Postgres `memory_signals` (scoped, idempotent, best-effort). SQLite `search()` applies signal boosts (BM25-scored, ladder-ranked, BM25-positive candidates only). Postgres `search()` does not yet apply `memory_signals` boosts — schema and ingest are now in place but search boost application remains open.
