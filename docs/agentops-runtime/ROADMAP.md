...
**In-progress note (2026-06-04, TDD — twentieth slice):** Postgres BM25 full-corpus exactness landed. Added `_postgres_keyword_candidates_sql()` — a scoped keyword-candidates query using `ts_vec @@ websearch_to_tsquery('english', %s)` with no `LIMIT`, preserving all seven `_PG_SCOPE_COLS` predicates and JSONB metadata filtering. Postgres `search()` now executes both the existing bounded `_postgres_search_sql()` (vector+FTS overfetch window) and the new unbounded keyword candidates SQL, merging rows by `record_id` before Python Okapi BM25 scoring. Exact keyword matches that fell outside the bounded SQL window are now always available for BM25 re-ranking. All downstream invariants preserved: only BM25-positive rows are returned or signal-boosted; metadata filtering and scope isolation apply to both queries; deterministic ordering by descending effective score then record_id; final `safe_limit` truncation after ranking. Two new RED→GREEN tests: (1) SQL-contract test confirming no `LIMIT %s`, all seven scope predicates, `ts_vec @@ websearch_to_tsquery`, and nullable JSONB metadata filter; (2) behavior test confirming an exact keyword match absent from the bounded SQL window appears in results when `conn.keyword_rows` is populated. Test evidence: 120/120 in `test_memory_record_store.py` pass; 106/106 regressions in `test_memory_records_api.py` + `test_memory_record_tools.py` + `test_remote_memory_record_backend.py` + `test_deep_memory_prefetch.py` pass; `ruff check` and `git diff --check` clean. M5B remains Started: production `embed_fn` injection/wiring remains open.
