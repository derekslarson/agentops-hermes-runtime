...
- BM25 exactness on Postgres: Postgres still uses `ts_rank` (PostgreSQL FTS) rather than Okapi BM25; local SQLite uses the Python `_bm25_scores` implementation. These produce different rank orderings for the same corpus — not yet unified.
