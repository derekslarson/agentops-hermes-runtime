...
**Completion note (2026-06-01):** M11 is complete. `LocalRunSupervisor` covers worker registration/capacity, drain/shutdown, warm conversation idle reuse, per-conversation queued turns, process-isolated submitted runs, leased run-to-completion jobs, cooperative run cancellation, max-runtime failure semantics, lease expiry/recovery, per-job cron leasing/concurrency, delivery failure handling, and RuntimeContext multiprocessing safety. Final profile evidence proves the run-to-completion lifecycle selects scoped backends for `local-multi`, `compose-self-hosted`, and `aws-managed` profiles without introducing cloud-specific core contracts. Test evidence: `python -m pytest tests/agent/test_runtime_supervisor.py tests/agent/test_runtime_context.py -q` → 66 passed, 1 known dependency deprecation warning; `python -m ruff check agent/runtime_supervisor.py tests/agent/test_runtime_supervisor.py` → passed; `git diff --check` → passed.
