...
- Extracted-signal boost parity: `RelationalMemoryRecordBackend` now ingests extracted signals into SQLite `memory_signals` (scoped, idempotent, best-effort). Boost application in search ranking, Postgres `memory_signals` schema/SQL/ingest path, cloud embedder packaging, and Postgres BM25 exactness remain open.
