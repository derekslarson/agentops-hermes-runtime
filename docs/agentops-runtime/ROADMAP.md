...
2. Idempotent scoped import endpoint or job so historical transcripts can be bulk-loaded into the remote store.
3. Managed-cloud profile registration (AWS/GCP backends register their own `DEEP_MEMORY` adapter under a non-compose profile).
