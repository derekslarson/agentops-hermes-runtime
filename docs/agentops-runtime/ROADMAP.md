...
**Autonomous run note (2026-06-01):** Started the worker fleet/run lifecycle slice by hardening the local contract implementations that distributed workers will rely on. `LocalRunLeaseBackend` now supports expiring leases with deterministic `now`/`lease_seconds` arguments so a different worker can recover a run/job/conversation lease after owner death. `LocalWorkerRegistry` now records capacity/capabilities, heartbeat timestamps, expiry, slot snapshots, drain state, `list_workers(...)`, and expired-worker recovery. `LocalRunSupervisor` now has an explicit drain request path: it marks the worker draining through the registry, refuses new run/turn submissions with failed `RunResult`s, and lets in-flight runs finish through normal shutdown. A later focused run added `LocalRunSupervisor.submit_turn(...)`, a bounded worker-local busy-turn queue that serializes turns for the same RuntimeContext conversation without consuming worker slots while waiting, applies per-conversation backpressure, fails pending queued turns on drain/shutdown without starting them, and still allows different conversations to use independent worker slots. A follow-up run added `submit_warm_turn(...)`, which keeps a conversation bound to its active warm run context until idle timeout, starts a fresh run after idle expiry while preserving durable conversation scope, and clears/refuses warm records during drain/shutdown. A later focused run added a backend-agnostic run-to-completion surface for non-conversation work: `run_to_completion(...)`/`submit_run_to_completion(...)` claim a `RunLeaseBackend` lease keyed per run/job before executing an event/manual/cron callable, record sanitized success/failure through the audit backend, release the lease on either path, and refuse a duplicate claim while a lease is live; `run_due_cron_once(...)` drives `CronBackend.claim_due`/`complete_run`/`fail_run` so each cron firing leases per job/firing rather than under a global run lock — a long builder firing whose lease is held is skipped while an unrelated watchdog that becomes due later is still claimed, executed, and completed, and drain refuses new run-to-completion/cron claims while active submitted work finishes. This is partial M11 groundwork only; remaining work includes subprocess-per-run execution, wiring this run-to-completion surface into the worker poll loop, and proving the same lifecycle across local-multi, compose-self-hosted, and AWS adapter profiles.
