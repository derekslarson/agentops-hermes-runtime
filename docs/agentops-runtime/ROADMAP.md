...
- Extracted-signal boost parity: `RelationalMemoryRecordBackend` ingests extracted signals into both SQLite and Postgres `memory_signals` (scoped, idempotent, best-effort), and both SQLite and Postgres `search()` now apply signal boosts to BM25-positive candidates returned by their ranked search windows. Remaining parity gap: Postgres signal boosts currently re-rank only the returned SQL window; candidate overfetch/pull-in support for signal-strong records outside that window remains open.
