...
**Progress note (2026-05-31, remote adapter slice):** Added a provider-neutral `HttpCronBackend` that implements the native `CronBackend` surface for Compose/cloud control planes: CRUD/list/history, due-job claim, lease renew/release, complete/fail run recording, RuntimeContext/delivery binding transport, fail-closed URL/timeout validation, sanitized job payloads, header-only bearer authentication, query-free scoped reads/deletes, malformed-response validation, and AgentOps initialization binding for non-local cron profiles with a fail-closed unavailable backend on misconfiguration. Test evidence: `python -m pytest tests/agent/test_remote_cron_backend.py tests/agent/test_runtime_cron_backend.py tests/agent/test_runtime_backends.py tests/agent/test_runtime_supervisor.py -q` → 73 passed; `python -m ruff check agent/agent_init.py agent/runtime_cron_http.py tests/agent/test_remote_cron_backend.py` → passed. M8 remains `Started`: the next slice still needs a durable remote/control-plane implementation or scheduler/worker execution path that proves recurring/one-shot/paused/restart/lease-timeout/drain/delivery semantics outside the client adapter.

**Progress note (2026-05-31, durable SQLite slice):** Added `SQLiteCronBackend`, a provider-neutral durable implementation of the logical `CronBackend` contract for local/Compose control-plane-style cron state. It persists scoped jobs, run history, and worker leases in SQLite with WAL/busy-timeout configuration and explicit `BEGIN IMMEDIATE` transactions around multi-statement claim/renew/finish mutations; durable cron scope survives new run/job IDs for the same conversation while remaining isolated across users/conversations. Restart coverage proves list/get, history, live lease duplicate prevention, expired-lease reclaim, paused/one-shot state, silent-output and delivery-error history, malformed `next_run_at` fail-closed state, and secret-safe job payload/update/error persistence. Test evidence: `python -m pytest tests/agent/test_runtime_cron_sqlite_backend.py -q` → 9 passed; `python -m pytest tests/agent/test_runtime_cron_backend.py tests/agent/test_runtime_backends.py tests/agent/test_runtime_supervisor.py -q` → 65 passed; `python -m ruff check agent/runtime_cron_sqlite.py tests/agent/test_runtime_cron_sqlite_backend.py` → passed. Independent review gates: spec PASS; quality/security APPROVED after transaction and secret-redaction fixes. M8 remains `Started`: end-to-end scheduler/worker execution through Compose/cloud queue adapters and full delivery routing proof are still pending.
