...
**Autonomous run note (2026-06-01):** Continued the worker fleet/run lifecycle slice. This run added optional subprocess-per-run isolation for submitted run callables via `LocalRunSupervisor(run_isolation="process")` while preserving the bound `RuntimeContext`; `RuntimeContext` is now pickle-safe across multiprocessing boundaries despite frozen metadata. The process pool is kept separate from the worker thread pool so queued turns and leased run-to-completion paths remain thread-backed and do not try to pickle registry/backend state. Current evidence includes `python -m pytest tests/agent/test_runtime_supervisor.py tests/agent/test_runtime_context.py -q` (63 passed), covering worker registration/capacity, drain/shutdown, warm conversations, queued turns, lock expiry/recovery, process-isolated submit runs, run-to-completion leases, per-job cron leasing/concurrency, delivery failure handling, and RuntimeContext multiprocessing safety. M11 is still not complete: remaining acceptance work is explicit cancel/max-runtime semantics for run-to-completion jobs and final lifecycle/profile evidence for local-multi, compose-self-hosted, and AWS adapter profile selection before marking this milestone Done.
