...
- Extracted-signal boost parity: `RelationalMemoryRecordBackend` now ingests extracted signals into SQLite `memory_signals` (scoped, idempotent, best-effort) and applies SQLite search boosts only to metadata-filtered, BM25-positive candidates. Postgres `memory_signals` schema/SQL/ingest/search boost, cloud embedder packaging, and Postgres BM25 exactness remain open.
