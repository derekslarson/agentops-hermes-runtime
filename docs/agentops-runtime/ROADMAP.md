...
4. **HTTP adapter coverage is partial.** Compose currently wires HTTP adapters for curated memory, cron, artifacts, audit, conversation routing, and run leases. Sessions, skills, credentials/secrets, queue, worker registry, delivery, and deep memory need explicit remote/durable adapters or control-plane endpoints before the first version is complete.
